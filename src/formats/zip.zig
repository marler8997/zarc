...
pub fn readInfo(TMP_reader: anytype) ReadInfoError(@TypeOf(TMP_reader))!Info {
    const file_length = try TMP_reader.context.getEndPos();
    ...
    find: while (pos > 0) {
        try TMP_reader.context.seekTo(pos);

        const read = try TMP_reader.readAll(&buffer);
        ...
        while (i < read - 4) : (i += 1) {
            if (std.mem.readIntLittle(u32, buffer[i..][0..4]) == EndCentralDirectoryRecord.Signature) {
                pos = pos + i;
                try TMP_reader.context.seekTo(pos + 4);
...
pub fn readDirectory(allocator: *std.mem.Allocator, TMP_reader: anytype, info: Info) ReadDirectoryError(@TypeOf(TMP_reader))!Directory {
    const Seek = Seeker(@TypeOf(TMP_reader));

    var headers = try std.ArrayListUnmanaged(CentralDirectoryHeader).initCapacity(allocator, info.num_entries);
    errdefer headers.deinit(allocator);

    var index: u32 = 0;
    try TMP_reader.context.seekTo(info.start_offset + info.directory_offset);

    var buffered = Seek.BufferedReader{ .unbuffered_reader = TMP_reader };
    ...
    while (index < info.num_entries) : (index += 1) {
        ...
        try hdr.readInitial(reader);
        if (hdr.disk_start != info.ecd.disk_number) return error.MultidiskUnsupported;
        try Seek.seekBy(TMP_reader, reader.context, @intCast(i64, hdr.filename_len + hdr.extrafield_len + hdr.file_comment_len));
    ...
    try Seek.seekTo(TMP_reader, reader.context, info.start_offset + info.directory_offset);

    for (headers.items) |*hdr| {
        try Seek.seekBy(TMP_reader, reader.context, 46);
        hdr.filename = try readFilename(&filename_buffer, reader, hdr.filename_len);
        try hdr.readSecondary(TMP_reader, reader);
    }

    std.sort.sort(CentralDirectoryHeader, headers.items, {}, centralHeaderLessThan);

    for (headers.items) |*hdr| {
        try Seek.seekTo(TMP_reader, reader.context, info.start_offset + hdr.offset);
        const signature = try reader.readIntLittle(u32);
        if (signature != LocalFileHeader.Signature) return error.MalformedLocalFileHeader;
        try hdr.local_header.read(hdr, TMP_reader, reader);
    }
...
pub const Directory = struct {
    ...
    pub fn readFileAlloc(self: Directory, TMP_reader: anytype, allocator: *std.mem.Allocator, index: usize) ![]const u8 {
        const Seek = Seeker(@TypeOf(TMP_reader));
        const header = self.headers.items[index];

        TMP_reader.context.seekTo(self.start_offset + header.local_header.offset);

        var buffer = try allocator.alloc(u8, header.uncompressed_size);
        errdefer allocator.free(buffer);

        var read_buffered = Seek.BufferedReader{ .unbuffered_reader = TMP_reader };
        var limited_reader = utils.LimitedReader(Seek.BufferedReader.Reader).init(read_buffered.reader(), header.compressed_size);
    ...
    pub fn extract(self: Directory, TMP_reader: anytype, dir: std.fs.Dir, options: ExtractOptions) !usize {
        const Seek = Seeker(@TypeOf(TMP_reader));
        ...
        extract: for (self.headers) |hdr| {
            ...
            try Seek.seekTo(TMP_reader, file_reader.context, self.start_offset + hdr.local_header.offset);

            var limited_reader = utils.LimitedReader(Seek.BufferedReader.Reader).init(file_reader, hdr.compressed_size);
            const reader = limited_reader.reader();
    ...
    pub fn getFileTree(directory: Directory, allocator: *std.mem.Allocator) !FileTree {
        var tree = FileTree{};
        try tree.entries.ensureTotalCapacity(allocator, @intCast(u32, directory.headers.items.len));
        errdefer tree.entries.deinit(allocator);

        for (directory.headers.items) |*hdr| {
